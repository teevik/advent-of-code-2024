...
pub fn part1(input: &str) -> i32 {
    ...
    let mut buttons_a_x = [0; LANES];
    let mut buttons_a_y = [0; LANES];
    let mut buttons_b_x = [0; LANES];
    let mut buttons_b_y = [0; LANES];
    let mut targets_x = [0; LANES];
    let mut targets_y = [0; LANES];

    let zero = Simd::<i32, LANES>::splat(0);
    let three = Simd::<i32, LANES>::splat(3);
    ...
    for _ in 0..CHUNKS {
        for i in 0..LANES {
            ...
            buttons_a_x[i] = a_x as i32;
            buttons_a_y[i] = a_y as i32;
            buttons_b_x[i] = b_x as i32;
            buttons_b_y[i] = b_y as i32;
            targets_x[i] = p_x;
            targets_y[i] = p_y;
        }

        let a_x = Simd::<i32, LANES>::from_array(buttons_a_x);
        let a_y = Simd::<i32, LANES>::from_array(buttons_a_y);
        let b_x = Simd::<i32, LANES>::from_array(buttons_b_x);
        let b_y = Simd::<i32, LANES>::from_array(buttons_b_y);
        let p_x = Simd::<i32, LANES>::from_array(targets_x);
        let p_y = Simd::<i32, LANES>::from_array(targets_y);
        ...
        let a = left_a / right;
        let b = left_b / right;
        let is_valid = (left_a % right).simd_eq(zero);

        let results = a * three + b;
        let results = results.as_array();

        let mut mask = is_valid.to_bitmask();

        while mask != 0 {
            let x = mask.trailing_zeros() as usize;

            sum += unsafe { *results.get_unchecked(x) };
